...
Here's a reference to the above, [CIT2002]_, and a
citation.
...
Here's a `hyperlink reference without a target <g>`_, which generates an
error.
...
document (a document-wide `TOC`_).
